...
app.config['UPLOAD_FOLDER'] = params['location_uploader'] 
app.config.update(
    MAIL_SERVER = 'smtp.gmail.com',
    MAIL_PORT = 465,
    MAIL_USE_SSL = True,
    MAIL_USERNAME = params['gmail-user'],
    MAIL_PASSWORD = params['gmail-pswrd']
)
mail = Mail(app)
...
class Contacts(db.Model):
    # '''s_no, name, email, phone_num, messege, date'''
...
def dashboard() :
    ...
    if request.method=='POST':
        #REDIRET TO ADMIN PANNEL
        username = request.form.get("uname")
        userpass = request.form.get("pass")
        if (username==params['admin_user'] and userpass==params['admin_pswrd']):
...
def contact() :
    ...
    if request.method == "POST":
        ...
        entry = Contacts(name=name, email=email, phone_num=phone, msg=messege, date=datetime.now() )
